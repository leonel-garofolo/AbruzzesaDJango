...
def e_inscripcion_socios(request):
    if request.is_ajax():
        q = request.GET.get('term', '')
        drugs = Alumno.objects.filter(
            Q(es_socio = True)
            & (Q(dni__icontains = q)
            | Q(nombre__icontains = q)
            | Q(apellido__icontains = q)))[:7]
...
def e_inscripcion(request):
    if request.is_ajax():
        q = request.GET.get('term', '')
        year = datetime.date.today().year
        drugs = Inscripcion.objects.filter(Q(id_alumno__es_socio = False) 
                & Q(fecha_alta__icontains=year) & (Q(id_alumno__dni__icontains = q)
